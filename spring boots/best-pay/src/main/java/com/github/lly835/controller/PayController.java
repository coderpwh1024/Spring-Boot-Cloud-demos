...
/**
    ...
    public PayResponse pay(@RequestBody WxPayReq req) {
        //支付请求参数
        PayRequest request = new PayRequest();
        request.setOrderId(req.getOrderId());
        request.setOrderAmount(req.getAmount());
...
/**
    ...
    public ModelAndView notify(@RequestBody String notifyData) {
        ...
        if (response.getPayPlatformEnum() == BestPayPlatformEnum.WX) {
            if(StringUtils.isNotBlank(response.getOrderId())){
                wxUserService.updateByOrderId(response.getOrderId());
            }
    ...
    @GetMapping("/test")
    @ResponseBody
    public Long test(String signature, Long echostr, Long timestamp, Long nonce) {
        return echostr;
    }
